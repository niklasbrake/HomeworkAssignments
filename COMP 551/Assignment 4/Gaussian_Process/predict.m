function [mu,sig] = predict(X_T,X,Y,psi,Network_Depth,sig_w,sig_b,K)
	...
	A = 1/norm(t,2)*tranpose(t)*K;
	mu = linsolve(A,inv_k,'posdef','true');
	% mu = K_xD * inv_K * t;
	% sig = K_xx - K_xD* (K + sig_eps^2*eye(N))*transpose(K_xD);
	sig = [];
